...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            # check if the authority label is null or not
            temp = line['binomialAuthority_label']
            
            if temp != "NULL":
                # clean the label
                label = line['rdf-schema#label']
                if label.find('(') != -1:
                    data[label.split('(')[0].strip()] = temp
                else:
                    data[label] = temp
...
def update_db(data, db):
  #  print data.keys()
 #   print data['rdf-schema#label']
    for key, value in data.items():
        db.arachnid.update({"label": key}, 
                        {"$set": {'classification':{'binomialAuthority': value}}})
